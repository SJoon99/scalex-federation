...
The ApplicationSet scans `releases/*/release.yaml` in `https://github.com/SmartX-Team/scalex-federation.git` at `main`. Releases are rendered only when `state: active` is present. Generated Applications use Argo CD multi-source Helm configuration and deploy through the `scalex` destination alias to the Karmada API, with the namespace supplied by each release.

...
3. Replace inline `helm.values` with the feature-specific environment values.
4. Confirm Infra dependencies and cross-cluster credentials already exist.
5. Change only that entry from `state: disabled` to `state: active`.
6. Review and merge the Federation PR.

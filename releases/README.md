...
읽어 실제 Argo `Application`을 생성한다. `schemaVersion: v1`은 지원하는 descriptor 계약을
선택하기 위한 필드이며 ApplicationSet도 이 값을 검사한다.
...
새 release를 만들 때 `schemaVersion: v1`을 선언하고 디렉터리명, descriptor `name`,
namespace와 source repository 이름을 같은 기능 repo 식별자로 맞춘다. values path도 해당
디렉터리를 정확히 가리켜야 한다.

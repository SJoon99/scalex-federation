...
- `poc/rgw-analysis-web`: 기존 Application identity와
  ...
  경계를 유지하며 검증된 `scalex-feature-poc` chart를 사용한다.

현재 active release는 하나다. 별도의 `cuty/rgw-analysis-web` Application을 만들지
않으며, `poc/rgw-analysis-web`의 source SHA와 `awsCli`/`nginx` image digest를 한 묶음으로
고정한다.
...
`poc/rgw-analysis-web`은 검증된 ScaleX feature commit과 public `awsCli`/`nginx`
digest를 `pinned` mode로 고정한다. 자동 promotion은 아직 적용하지 않는다.

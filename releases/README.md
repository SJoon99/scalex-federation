...
`rgw-analysis-web`은 CI 도입 전 수동 검증 단계에서 exact child commit과 Docker Hub
`flow`/`web` digest를 `pinned` mode로 고정한다. `pinned` release는 자동 promotion의
대상이 아니며, Argo/Karmada 동작 확인 후 GHCR publication을 도입할 때 canonical
`sha-<commit>` image 좌표로 변경하고 `tracked` mode를 활성화한다.

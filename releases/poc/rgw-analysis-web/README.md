...
1. B Infra Argo가 OBC를 sync하고 `Bound`와 같은 이름의 Secret/ConfigMap을 확인한다.
2. Argo가 Federation binding 명세와 workload Karmada policy를 sync한다.
3. management plane에서 공통 runner를 `--all` 또는 이 binding만 지정해 실행한다.
